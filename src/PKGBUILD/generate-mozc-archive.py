...
def main():
    ...
    if not os.path.exists(f'mozc-fcitx-{date}.zip'):
        subprocess.run(['wget', 'https://github.com/fcitx/mozc/archive/refs/heads/fcitx.zip', '-O', f'mozc-fcitx-{date}.zip'], check=True)

    if os.path.exists('mozc-fcitx'):
        shutil.rmtree('mozc-fcitx')

    # mozc-fcitx を展開して、mozc-fcitx/src/unix を mozc に移動
    subprocess.run(['unzip', '-q', f'mozc-fcitx-{date}.zip', 'mozc-fcitx/src/unix/*'], check=True)
    shutil.rmtree(mozcdir + '/src/unix')
    os.rename('mozc-fcitx/src/unix', mozcdir + '/src/unix')
    shutil.rmtree('mozc-fcitx')

    # mozc のアーカイブを作成
    subprocess.run(['tar', '--zstd', '-cf', f'{mozcdir}.tar.zst', mozcdir], check=True)
